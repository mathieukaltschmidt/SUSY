...
\section{The Standard Model — Basics}
\begin{frame}{Standard Model — The Basics}
\only<1, 2, 4>{The gauge group of the Standard Model (SM) is
\begin{align}
  \mathcal{G}_\mathrm{SM}&=\su(3)_C\times\su(2)_T\times\un(1)_Y.
\end{align}}
\only<3>{
Beware! We only use lefthanded fields $\psi_L=\vect{\psi_\alpha\\0}$, righthanded fields are included via charge conjugation $\left(\psi_R\right)^c=\vect{0\\\overline{\psi}^{\dot{\alpha}}}^c=\vect{\psi_\alpha\\0}.$
\vfill
...
\begin{align}
  \mathcal{L}_\mathrm{Higgs}\supset-\left(D^\mu\phi\right)^\dagger\left(D_\mu\phi\right)+\mu\phi^\dagger\phi-\lambda\left(\phi^\dagger\phi\right)^2-\lambda^\psi\left[\overline{\psi}\phi\psi\right]_\numb{1}.
\end{align}
\only<2>{
The fermions gain masses \mbox{$m_\psi=v\lambda^\psi$} when the Higgs field acquires its vacuum expectation value (VEV):
\begin{align*}
  \left\langle\phi\right\rangle&=\left(\begin{array}{c}0\\v\end{array}\right)\kern4em\left(\text{shorthand: }\left\langle\phi_0\right\rangle=v\right).
\end{align*}
}
\only<3, 4>{
For our three types of massive fermions (electron, up-quark, down-quark) the corresponding singlets look like:\footnote{Careful, since $\mytilde{\phi}=\epsilon\phi^*$ to account for the u-type quarks.}
\begin{align}
\lambda^u\left[\overline{q}_L\mytilde{\phi}u_R\right]_\numb{1}\quad\quad\lambda^d\left[\overline{q}_L\phi d_R\right]_\numb{1}\quad\quad\lambda^e\left[\overline{\ell}_L\phi e_R\right]_\numb{1}.
\end{align}
\only<4>{
Furthermore, the SM consists of three generations of fermions thus promoting the $\lambda^\psi$ to complex 3x3 matrices $\lambda^e_{mn}$, $\lambda^d_{mn}$ and $\lambda^u_{mn}$. The $\lambda^f_{mn}$ can be diagonalised via bi-unitary transformations:
\begin{align}
  V_f^\dagger\lambda^fU_f&\propto\operatorname{diag}\left(m^{(1)}_f, m^{(2)}_f, m^{(3)}_f\right).
\end{align}
}
}
\end{frame}
...
\only<3>{The Standard Model of Particle Physics has 19 free parameters with $v$ being the only one carrying a physical dimension.\footnote{15 real parameters, 3 mixing angles, 1 CP-violating phase.}}
\end{frame}
...
\end{align}
\only<1>{What terms are contained in $W$?}
\only<2, 3, 4>{
In general, the superpotential contains a great variety of different terms, under them the Yukawa couplings:
\begin{align}\nonumber
  W=&\lambda_d\left[H_dQU\right]_\numb{1}+\lambda_d\left[H_dLE\right]_\numb{1}-\lambda_u\left[H_uQU\right]_\numb{1}+\mu \left[H_uH_d\right]_\numb{1}\\
  &+a\left[\myhat{L}\myhat{H_u}\right]_\numb{1}+b\left[\myhat{Q}\myhat{L}\myhat{D}\right]_\numb{1}+c\left[\myhat{U}\myhat{U}\myhat{D}\right]_\numb{1}+d\left[\myhat{L}\myhat{L}\myhat{E}\right]_\numb{1}.
\end{align}
}
\only<3>{
The terms in the last line introduce $B$-number violation via proton decay as well as lepton number violation, but by imposing $R$-parity
\begin{align}
  R=(-1)^{3(B-L)+2s},
\end{align}
we can get rid of them. Beware! This is not obligatory!
...
\only<4>{
We could use \alert{matter parity}
\begin{align}
P_\mathrm{m}&=(-1)^{3(B-L)},
\end{align}
...
\begin{align}\nonumber
  -\mathcal{L}^\mathrm{MSSM}_\mathrm{soft}\supset&\frac12M_i\mytilde{\lambda}_i\mytilde{\lambda}_i+M^2_{\mytilde{F}}\mytilde{f}^\dagger\mytilde{f}\\
  &+m_1^2H_d^\dagger H_d+m_2^2H_u^\dagger H_u+m_{12}^2\left(H_uH_d+\hc\right)\\\nonumber
  &+T_UH_uQU+T_DH_dQD+T_EH_dLE+\hc
\end{align}
Often, a parametrisation $m_{12}^2=\mu B$ (and $T_F=\lambda_f A_F$) is chosen. Therefore, the corresponding terms are called A and B-terms.
\end{frame}

\begin{frame}{MSSM — Note on the Higgs sector}
Repeat the SM steps:
\only<1, 2>{
In the MSSM the quartic coupling is generated by the kinetic and soft SUSY breaking terms leading to an effective Higgs potential.
\begin{align}\nonumber
...
\only<2>{
The two doublets acquire separate VEVs:
\begin{align}
...
\begin{frame}{MSSM — Mixing in the particle spectrum}
\only<1, 2, 3, 4, 5>{The SM particle spectrum looks like:}
\only<1>{
\begin{table}
...
$\color{olive}{\mytilde{u}_L}$&$\color{olive}{\mytilde{u}_R}$&$\color{olive}{\mytilde{c}_L}$&$\color{olive}{\mytilde{c}_R}$&$\color{olive}{\mytilde{t}_L}$&$\color{olive}{\mytilde{t}_R}$&&$\color{red}{\mytilde{B}}$&$\color{red}{\mytilde{W}^0}$\\
$\color{cyan}{\mytilde{d}_L}$&$\color{cyan}{\mytilde{d}_R}$&$\color{cyan}{\mytilde{s}_L}$&$\color{cyan}{\mytilde{s}_R}$&$\color{cyan}{\mytilde{b}_L}$&$\color{cyan}{\mytilde{b}_R}$&&$\mytilde{g}$&$\color{orange}{\mytilde{W}^\pm}$\\\\
$\color{purple}{\mytilde{e}_L}$&$\color{purple}{\mytilde{e}_R}$&$\color{purple}{\mytilde{\mu}_L}$&$\color{purple}{\mytilde{\mu}_R}$&$\color{purple}{\mytilde{\tau}_L}$&$\color{purple}{\mytilde{\tau}_R}$&&$\color{orange}{\mytilde{H}_d^-}$&$\color{red}{\mytilde{H}^0_d}$\\
$\color{brown}{\mytilde{\nu}_e}$&&$\color{brown}{\mytilde{\nu}_\mu}$&&$\color{brown}{\mytilde{\nu}_\tau}$&&&$\color{red}{\mytilde{H}_u^0}$&$\color{orange}{\mytilde{H}^+_u}$
\end{tabular}
}\end{table}
}
\only<5>{
...
{\setlength{\extrarowheight}{5pt}
\begin{tabular}{ccccccccc}
$\color{olive}{\mytilde{q}_1}$&$\color{olive}{\mytilde{q}_2}$&$\color{olive}{\mytilde{q}_3}$&$\color{olive}{\mytilde{q}_4}$&$\color{olive}{\mytilde{q}_5}$&$\color{olive}{\mytilde{q}_6}$&&$\color{orange}{\mytilde{\chi}^\pm_1}$&$\color{orange}{\mytilde{\chi}^\pm_2}$\\
$\color{cyan}{\mytilde{q}'_1}$&$\color{cyan}{\mytilde{q}'_2}$&$\color{cyan}{\mytilde{q}'_3}$&$\color{cyan}{\mytilde{q}'_4}$&$\color{cyan}{\mytilde{q}'_5}$&$\color{cyan}{\mytilde{q}'_6}$&&$\mytilde{g}$&\\\\
$\color{purple}{\mytilde{\ell}_1}$&$\color{purple}{\mytilde{\ell}_2}$&$\color{purple}{\mytilde{\ell}_3}$&$\color{purple}{\mytilde{\ell}_4}$&$\color{purple}{\mytilde{\ell}_5}$&$\color{purple}{\mytilde{\ell}_6}$&&$\color{red}{\mytilde{\chi}^0_1}$&$\color{red}{\mytilde{\chi}^0_2}$\\
$\color{brown}{\mytilde{\nu}_1}$&&$\color{brown}{\mytilde{\nu}_2}$&&$\color{brown}{\mytilde{\nu}_3}$&&&$\color{red}{\mytilde{\chi}^0_3}$&$\color{red}{\mytilde{\chi}^0_4}$
\end{tabular}
}\end{table}
}
\only<6>{
...
\begin{itemize}
\item The charged gauginos ($\mytilde{W}^\pm$, $\mytilde{H}_u^+$, $\mytilde{H}_d^-$) form the charginos $\mytilde{\chi}^\pm_i$.
\item The neutral gauginos ($\mytilde{B}$, $\mytilde{W}^0$, $\mytilde{H}_u^0$, $\mytilde{H}_d^0$) form the neutralinos $\mytilde{\chi}^0_i$.
\item The squarks ($\mytilde{q}_{i, L}$, $\mytilde{q}_{i, R}$) form mass eigenstates labeled $\mytilde{q}_i$.
\item The sleptons ($\mytilde{e}_{i, L}$, $\mytilde{e}_{i, R}$) form eigenstates $\mytilde{\ell}_i$.
\item The Higgs bosons ($H_d^-$, $H_d^0$, $H_u^0$, $H_u^+$) form: a charged pair $H^\pm$, two CP-even neutral scalars $h^0$, $H^0$ and a CP-odd $A^0$.
\end{itemize}
...
\item 5 hermitian scalar mass matrices $M^2_{\mytilde{F}}$ (5x9 parameters) 
...
\end{itemize}}
\vfill
\only<4>{The full Minimal Supersymmetric Standard Model has 124 free parameters\footnote{Consisting of 3 couplings, 37 real masses, 39 mixing angles and 45 CP-violating phases.} (MSSM-124).}
